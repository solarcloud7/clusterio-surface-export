...
//
// The fixture-check sections are EXPORTED so the unified golden-batch runner
// (tests/lab-gallery/run-golden-batch.mjs) can compose them in a single golden-pair load; this file's
// own main() remains the standalone single-fixture driver.
//
// Usage:
...
import { createBatchLifecycle, rcon, lua } from "../lab-gallery/batch-lifecycle.mjs";
...
export function readInserter(host) {
	return lua(host, `
...
// --- Exported sections (composed by the unified golden-batch runner) ------------------------------

// The ADVERSARIAL STATE gate: the loaded GOLDEN DESTINATION (host-2) must have bonus 0 — ASSERT it
// (never force it). Runs right after the golden load, before any measurement touches the pair.
export function b7AssertAdversarialDest(results) {
...
// Fingerprint-verify the source fixture, run the production whole-omnibus /transfer-platform, wait for
// the terminal record, then adjudicate with INDEPENDENT physical destination reads. Needs the
// config-bound lifecycle L (dropMarker/waitForImportResult are golden-save-scoped).
export async function b7TransferSection(L, fixture, results, ids) {
	...
	const importMarker = L.dropMarker(2, "transfer-import");
	...
	const terminal = await L.waitForImportResult(2, importMarker);
...
export function renderB7Notebook(results) {
	const L2 = [];
...
async function main() {
	...
	const fixture = manifest.fixtures.find(f => f.id === B7_FIXTURE_ID);
	if (!fixture) throw new Error(`fixture ${B7_FIXTURE_ID} missing from gallery manifest`);
	const results = {
	...
	try {
		...
		if (runs("load")) {
			...
			b7AssertAdversarialDest(results);
			results.goldenLoaded = true;
		}

		if (runs("transfer")) {
			await b7TransferSection(L, fixture, results, ids);
		}
	} catch (error) {
		...
		if (!noNotebook && !restoreOnly && sections.length === ALL_SECTIONS.length) {
			appendFileSync(NOTEBOOK, renderB7Notebook(results));
		}
