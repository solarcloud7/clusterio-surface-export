...
| transfer-fidelity | KEEP: the meter-drift SENTINEL (serialized-expected vs SOURCE physical census). SC-6 Phase 3 shipped the paired-reads census live (`census-omission-abort` pad proves it fires + fail-closes on a real transfer), but the census's clean-transfer verdict is not yet exposed as a dest-readable artifact — delete transfer-fidelity only once SC-6 Phase 4 adds that positive telemetry witness |
