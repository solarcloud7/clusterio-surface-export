...
export function createCase(context) {
	...
	const ATTRS = [
		...
		},
		{
			...
			write: `e.name_tag = "${LABEL_PREFIX}-wall"`, read: STRING_READ("name_tag"), expect: `${LABEL_PREFIX}-wall`,
		},
		{
			key: "name_tag_loader", attribute: "name_tag", on: "loader",
			write: `e.name_tag = "${LABEL_PREFIX}-loader"`, read: STRING_READ("name_tag"), expect: `${LABEL_PREFIX}-loader`,
		},
		{
		...
		},
		{
			...
			write: `e.display_panel_text = "${LABEL_PREFIX}-panel"`,
			read: STRING_READ("display_panel_text"), expect: `${LABEL_PREFIX}-panel`,
		...
		},
		{
			...
			write: 'e.alert_parameters = { show_alert = true, show_on_map = false, '
				+ `icon_signal_id = { type = "item", name = "iron-plate" }, alert_message = "${LABEL_PREFIX}-alert" }`,
			...
			expect: `true|false|${LABEL_PREFIX}-alert|item:iron-plate:normal`,
		},
		{
			...
			expect: `yellow|/${LABEL_PREFIX}-status`,
		},
		{
		...
		},
		{
			...
			write: `e.character_corpse_death_cause = { "", "${LABEL_PREFIX}-death" }`,
			read: "return localised_key(e.character_corpse_death_cause)",
			expect: `|${LABEL_PREFIX}-death`,
		},
		{
	...
	let built, placementById, exercisable, surviving;
