...
local keep={['space-platform-hub']=true,['fusion-reactor']=true,['fusion-generator']=true}
for _,entity in pairs(p.surface.find_entities_filtered({})) do
    if entity.valid and not keep[entity.name] then entity.destroy() end
end
local isolated=nil
...
rcon.print(helpers.table_to_json({entities=#p.surface.find_entities_filtered({}),control=control and control.amount or 0,managed=managed,reactors=#reactors}))
"@
    $fixtureData = $fixture | ConvertFrom-Json
    if ($fixtureData.control -ne 5 -or $fixtureData.managed -le 0) {
