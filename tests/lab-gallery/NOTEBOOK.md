...
## 2026-07-20 — SC-69 wave 1: the 5 sabotage teeth are pad fixtures; legacy runners DELETED

- Schema: lifecycle ops/checks gained `end: source|dest`; `expect: "gate-failure"` (requires a dest-end arm_hook/mutate_force sabotage + an explicit source-end physical_read as the source-preserved witness; dest-end lua REJECTED — review OBS-1). New op spawn_fluid; reset_mutable clears fluids; new read surface_entity_count (forward-looking, unused).
- Remotes: lifecycle_dest_setup (dest-end sabotage recorded in dest storage for guaranteed disarm); source-after-act runs source-end checks against a preserved scratch. Orchestrator branches on expect: gate-failure polls the dest copy AWAY and asserts the source scratch PRESERVED + physically intact.
- 5 new pads (slots 8-64, 36-50, 36-64, 64-36, 64-50): gate-item-loss, gate-fluid-loss, rollback-validation-failure, failed-entity-attribution, force-bonus-held. Gallery force bonus measured 11; baked bulk-inserter hand seats 8.
- MEASURED CONTRACT CORRECTION: test_force_entity_failure "inventory_and_fluid:<name>" requires items AND fluids (a chest never matches — first attempt armed a hook that could not fire); value=true targets the first inventory-bearing entity. And a FIRED entity-failure hook forces the verdict FAIL-SAFE (failedStage "test_hook", source preserved) with clean attribution (failedEntityLosses.total_items=100, totalItemLoss=0) — the fixture encodes this measured behavior; the legacy test's "validation passes" expectation predates the Pitfall #30 hardening.
