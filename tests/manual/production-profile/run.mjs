...
import { recoveryCase, summary } from "../transfer-reliability/cases.mjs";
...
if (mode === "--analyze") {
  const result = analyzeProfile(JSON.parse(readFileSync(input)));
  ...
  const report = { schemaVersion: 1, run, startedAt: new Date().toISOString(), runtime, cleanup: { success: false }, hashes: {} };
  ...
  try {
    ...
    try { Object.assign(report, analyzeProfile(report)); } catch (error) { report.error ||= error.stack; report.verdict = "HARNESS_ERROR"; }
