...
import { ProductionLab } from '../production-profile/lab.mjs';
import { resolvedAdmissions } from './upload-review-cases.mjs';

test('production commands and waits inherit cancellation before invoking Docker or polling', async () => {
 const lab=Object.create(ProductionLab.prototype);
 lab.cancelled=true;
 assert.throws(()=>lab.command(['instance','list']),/interrupted/i);
 await assert.rejects(lab.until(()=>{throw Error('poll must not run');},'cancelled poll'),/interrupted/i);
});

test('admission acceptance stays incomplete until the checkpoint sender has reconciled', async () => {
 for(const failure of ['checkpoint','load','epoch',null]) {
  const report={cases:[]},saved=[];
  let loaded=false;
  const lab={run:'fixture',lua:(_instance,code)=>({result:
   code.includes('return {success=true,receipts=saved}')?{receipts:[]}:
   code.includes('replies=replies')?{copies:4,replies:Array(4).fill({state:'accepted'})}:
   code.includes('state=r.state')?{state:'receiving'}:{epoch:loaded?'new':'old'}}),
   until:async read=>{if(failure==='epoch')throw Error('epoch');assert.ok(read());},
   checkpoint:async()=>{if(failure==='checkpoint')throw Error('checkpoint');return {};},
   load:async()=>{if(failure==='load')throw Error('load');loaded=true;}};
  const result=resolvedAdmissions(lab,report,()=>saved.push(structuredClone(report.cases[0])));
  if(failure)await assert.rejects(result,new RegExp(failure));else await result;
  assert.ok(saved.every(row=>row.status!=='PASS'||row.senderReconciled===true));
  assert.equal(report.cases[0].status,failure?'RUNNING':'PASS');
 }
});
...
test('browser cleanup failure and Docker cleanup exception both retain evidence',async()=>{
 ...
 assert.match(f.report.error,/browser close/);
 assert.match(f.report.harnessErrors[1],/docker cleanup/);
});
