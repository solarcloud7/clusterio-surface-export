...
export const archiveLimit = (profile, suffix) => (profile === "production" && suffix === "client" ? 8 : 2) * 1024 ** 3;
async function digest(file, limit) {
  assert.ok(fs.statSync(file).size <= limit, `archive exceeds ${limit} byte bound`);
...
export async function storageAction(action, suffix, run, expected, profile = "lab") {
  ...
  const limit = archiveLimit(profile, suffix);
  ...
  if (action === "backup") {
    ...
    return {suffix, sha256: await digest(archive, limit), bytes: fs.statSync(archive).size, compared: true};
  }
  assert.equal(await digest(archive, limit), expected, "archive changed before destructive action");
  if (action === "erase") {
  ...
  tar(["--compare", "-f", archive, "-C", "/data"]);
  return {suffix, sha256: await digest(archive, limit), compared: true};
}
