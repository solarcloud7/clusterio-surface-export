...
//
// The measure section is EXPORTED so the unified golden-batch runner
// (tests/lab-gallery/run-golden-batch.mjs) can compose it in a single golden-pair load; this file's
// own main() remains the standalone single-fixture driver. NOTE for composers: the measure MUTATES the
// source world (seats iron-plate x1 on the pair inserter) — a subsequent whole-omnibus transfer in the
// same load carries that plate, and the exact gate must conserve it.
//
// Usage:
...
import { createBatchLifecycle, DOUBLE_EPSILON, lua } from "../lab-gallery/batch-lifecycle.mjs";
...
export function readPair(host) {
	return lua(host, `
		...
		return {success=true,platformIndex=surf.index,
			...
			heldName=(hs~=nil and hs.valid_for_read) and hs.name or nil,
			heldCount=(hs~=nil and hs.valid_for_read) and hs.count or 0,
...
function measureBaked(host) {
	const result = lua(host, `return remote.call('surface_export','no_tick_sync_selftest',` +
		...
		`held={name='${B8_HELD.name}',count=${B8_HELD.count},quality='${B8_HELD.quality}'}})`);
...
// --- Exported section (composed by the unified golden-batch runner) -------------------------------

// The full measure body: fingerprint-verify the pair on the loaded source, run 1 (normal, fresh
// seating), run 2 (tick-paused, idempotent-skip). Pushes any unpause failure onto results.errors and
// sets results.fingerprint/run1/run2/tickPauseRestored/verdict.
export async function b8MeasureSection(fixture, results) {
	...
	if (!(r2.held_after && r2.held_after.name === B8_HELD.name && r2.held_after.count === B8_HELD.count)) {
...
export function renderB8Notebook(results) {
...
async function main() {
	...
	const fixture = manifest.fixtures.find(f => f.id === B8_FIXTURE_ID);
	if (!fixture) throw new Error(`fixture ${B8_FIXTURE_ID} missing from gallery manifest`);
	const results = {
	...
	try {
		...
		if (runs("measure")) {
			await b8MeasureSection(fixture, results);
		}
	} catch (error) {
		...
		if (!noNotebook && !restoreOnly && sections.length === ALL_SECTIONS.length) {
			appendFileSync(NOTEBOOK, renderB8Notebook(results));
		}
...
// Import-safe: main runs only when this file is the invoked script (the unified golden-batch runner
// imports the exported sections without triggering a standalone batch).
