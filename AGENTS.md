...
| `node tools/clusterio/reconcile.mjs plan\|apply --cluster <dev\|name> --desired <file> [--yes] [--restart]` | Desired-state file (e.g. `tools/clusterio/desired/vm.json`); mod ZIPs in `docker/seed-data/mods`; cluster access as for `ctl.mjs`; the `surface-export gateways`/`set-gateway-links` commands for gateway links | Exact clusterioctl actions for stored mods, the desired mod pack, controller, host and instance config and gateway links; `apply --yes` runs them and reports configuration and runtime separately (exit 0 converged, 4 restart pending) | Replace a stored mod version, set empty values, restart stopped instances, verify what running games loaded, or authorize applying to a shared or production cluster |
