...
| `node tools/clusterio/reconcile.mjs plan\|apply --cluster <dev\|name> --desired <file> [--yes] [--restart]` | Desired-state file (e.g. `tools/clusterio/desired/vm.json`); mod ZIPs in `docker/seed-data/mods`; cluster access as for `ctl.mjs` | Exact clusterioctl actions for mods, the desired mod pack, controller and instance config; `apply --yes` runs them in order and re-plans | Delete stored mods, other packs or instances, manage gateway links, or authorize applying to a shared or production cluster |
