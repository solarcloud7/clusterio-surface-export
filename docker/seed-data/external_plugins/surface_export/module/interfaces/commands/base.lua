...
--- resolves to its `.index` (and its current `.name` is returned as the 3rd value, for display messages); an
--- orphaned lock (the platform was already deleted) is recovered by a name scan of the registry. Fails loud
--- on an ambiguous name (≥2 live platforms — or ≥2 locks — share it) so a destructive lock op never silently
--- targets the wrong platform; the caller prints the error and returns.
--- @param force LuaForce
--- @param name_or_index string|number: Unique platform index (preferred) or display name
--- @return number|nil lock_key, string|nil err, string|nil display_name
