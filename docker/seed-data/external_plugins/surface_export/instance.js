...
class InstancePlugin extends BaseInstancePlugin {
    controllerManagedTransferExports = new Set();
    ...
    async sendConfigurationToLua() {
        try {
            ...
            this.logger.warn(`Failed to send configuration to Lua: ${(0, helpers_1.getErrorMessage)(err)}`);
        }
    }
    /**
    ...
    async handleExportComplete(data) {
        ...
        try {
            ...
            if (data.destination_instance_id) {
                ...
                const transferResponse = await this.instance.sendTo("controller", new messages.TransferPlatformRequest({
                    exportId,
            ...
            if (this.pendingTransfer && this.pendingTransfer.job_id === data.job_id) {
                ...
                if (transferResponse.success) {
                    ...
                    await this.sendRcon(`/sc local SurfaceLock = require("modules/surface_export/utils/surface-lock"); ` +
            ...
            this.logger.error(`Error handling export completion: ${(0, helpers_1.getErrorMessage)(err)}`);
        }
    }
    /**
    ...
    async handlePlatformStateChanged(data) {
        try {
            ...
            this.logger.warn(`Platform state change notification failed: ${(0, helpers_1.getErrorMessage)(err)}`);
        }
    }
    /**
    ...
    async handleTransferRequest(data) {
        ...
        try {
            ...
            this.logger.error(`Error handling transfer request: ${(0, helpers_1.getErrorMessage)(err)}`);
        }
    }
    /**
    ...
    async handleImportFileRequest(data) {
        ...
        try {
            const result = await this.importPlatformFromFile(String(data.filename || ""), typeof data.platform_name === "string" ? data.platform_name : null, typeof data.force_name === "string" ? data.force_name : "player");
            if (result.success) {
            ...
            this.logger.error(`Error handling import file request: ${(0, helpers_1.getErrorMessage)(err)}`);
        }
    }
    /**
    ...
    async exportPlatform(platformIndex, forceName = "player", targetInstanceId = null) {
        ...
        try {
            ...
            const rconResult = await this.sendRcon(`/sc local export_id, err = remote.call("surface_export", "export_platform", ${platformIndex}, "${lib.escapeString(forceName)}", ${targetArg}); ` +
            ...
            this.logger.error(`Export failed: ${(0, helpers_1.getErrorMessage)(err)}`);
            return { success: false, error: (0, helpers_1.getErrorMessage)(err) };
        }
    }
    async waitForExportData(exportId, timeoutMs = EXPORT_POLL_TIMEOUT_MS, intervalMs = EXPORT_POLL_INTERVAL_MS) {
        ...
        try {
            ...
            this.logger.error(`Failed to list available exports: ${(0, helpers_1.getErrorMessage)(listErr)}`);
        }
        return null;
    }
    /**
    ...
    async getExportData(exportId, options = {}) {
        try {
            ...
            const result = await this.sendRcon(`/sc rcon.print(remote.call("surface_export", "get_export_json", "${safeExportId}"))`);
            ...
            this.logger.error(`Get export data failed: ${(0, helpers_1.getErrorMessage)(err)}`);
            return null;
        }
    }
    /**
    ...
    async listExports() {
        try {
            const result = await this.sendRcon("/sc rcon.print(remote.call(\"surface_export\", \"list_exports_json\"))");
            return JSON.parse(result);
        }
        catch (err) {
            this.logger.error(`List exports failed: ${(0, helpers_1.getErrorMessage)(err)}`);
            return [];
        }
    }
    /**
    ...
    async listPlatforms(forceName = "player") {
        ...
        try {
            const result = await this.sendRcon(`/sc rcon.print(remote.call("surface_export", "list_platforms_json", "${safeForceName}"))`);
            ...
            this.logger.error(`List platforms failed: ${(0, helpers_1.getErrorMessage)(err)}`);
            return [];
        }
    }
    /**
    ...
    async importPlatform(exportData, forceName = "player") {
        ...
        try {
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                await this.sendRcon(`/sc remote.call("surface_export", "import_platform_chunk", ` +
                    `"${escapedPlatformName}", '${chunk}', ${chunkNum}, ${totalChunks}, "${escapedForceName}")`, true);
            ...
            this.logger.error(`Import failed: ${(0, helpers_1.getErrorMessage)(err)}`);
            return { success: false, error: (0, helpers_1.getErrorMessage)(err) };
        }
    }
    /**
    ...
    async importPlatformFromFile(filename, platformName = null, forceName = "player") {
        ...
        try {
            ...
            const scriptOutputPath = path_1.default.join(instanceDir, "script-output", filename);
            this.logger.verbose(`Reading file from: ${scriptOutputPath}`);
            const fileContent = await fs_1.default.promises.readFile(scriptOutputPath, "utf8");
            ...
            const escapedForceName = lib.escapeString(forceName);
            await (0, helpers_1.sendChunkedJson)(this.instance, `remote.call("surface_export", "import_platform_chunk", "${escapedTargetName}", %CHUNK%, %INDEX%, %TOTAL%, "${escapedForceName}")`, exportData, this.logger, RCON_CHUNK_SIZE);
            ...
            try {
                const result = await this.sendRcon("/sc rcon.print('{\"success\":true}')");
                ...
                this.logger.warn(`Could not verify import: ${(0, helpers_1.getErrorMessage)(verifyErr)}`);
            }
            return { success: true };
        }
        catch (err) {
            this.logger.error(`Import from file failed: ${(0, helpers_1.getErrorMessage)(err)}`);
            return { success: false, error: (0, helpers_1.getErrorMessage)(err) };
        }
    }
    /**
    ...
    async ensureLuaConsoleUnlocked() {
        ...
        for (let attempt = 1; attempt <= 2; attempt += 1) {
            try {
                ...
                this.logger.warn(`RCON handshake attempt ${attempt} failed: ${(0, helpers_1.getErrorMessage)(err)}`);
            }
        }
    ...
    async handleImportCompleteValidation(data) {
        ...
        if (!transferId || !Number.isInteger(sourceInstanceId) || sourceInstanceId <= 0) {
            if (operationId) {
                try {
                    ...
                    this.logger.error(`Failed to forward import operation completion for ${operationId}: ${(0, helpers_1.getErrorMessage)(emitErr)}`);
                }
                return;
            }
        ...
        try {
            ...
            const validationResult = await this.sendRcon(`/sc rcon.print(remote.call("surface_export", "get_validation_result_json", "${lib.escapeString(String(data.platform_name || ""))}"))`);
            ...
            this.logger.error(`Error during validation: ${(0, helpers_1.getErrorMessage)(err)}`);
            ...
            try {
                await this.instance.sendTo("controller", new messages.TransferValidationEvent({
                    ...
                    validation: {
                        itemCountMatch: false,
                        fluidCountMatch: false,
                        mismatchDetails: `Validation error: ${(0, helpers_1.getErrorMessage)(err)}`,
                    },
                }));
                this.logger.info(`Sent failure validation for transfer ${transferId} due to error`);
            }
            catch (sendErr) {
                this.logger.error(`Failed to send failure validation: ${(0, helpers_1.getErrorMessage)(sendErr)}`);
            }
        }
    }
    /**
    ...
    async handleDeleteSourcePlatform(request) {
        ...
        try {
            const result = await this.sendRcon(`/sc ` +
            ...
            this.logger.error(`Error deleting platform: ${(0, helpers_1.getErrorMessage)(err)}`);
            return { success: false, error: (0, helpers_1.getErrorMessage)(err) };
        }
    }
    /**
    ...
    async handleUnlockSourcePlatform(request) {
        ...
        try {
            ...
            this.logger.error(`Error unlocking platform: ${(0, helpers_1.getErrorMessage)(err)}`);
            return { success: false, error: (0, helpers_1.getErrorMessage)(err) };
        }
    }
    /**
    ...
    async handleTransferStatusUpdate(request) {
        ...
        try {
            ...
            await this.sendRcon(`/sc game.print("${request.message}", ${colorCode})`, true);
            return { success: true };
        }
        catch (err) {
            this.logger.error(`Error displaying transfer status: ${(0, helpers_1.getErrorMessage)(err)}`);
            return { success: false, error: (0, helpers_1.getErrorMessage)(err) };
        }
    }
