...
test('missing or malformed receiver limits prevent any upload',async()=>{
 for(const invalid of [undefined,{}, {...limits,chunkBytes:0},{...limits,maxSessions:1.5},
  ...
  const calls=[];
  const client=new UploadSessions(async(action,q)=>{calls.push(action);return {version:1,success:true,epoch:q.epoch,limits:invalid};});
...
test('late initialization cannot replace a newer runtime or revive a stopped one',async()=>{
 for(const replacement of [true,false]) {
  ...
  if(replacement) {
   const newer=client.initialize('new');pending[1].resolve({version:1,success:true,epoch:'new',highWater:20,limits});await newer;
  }else client.stop();
