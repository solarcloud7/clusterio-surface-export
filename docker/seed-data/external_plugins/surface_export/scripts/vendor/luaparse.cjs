// VENDORED: luaparse 0.3.1 (https://github.com/fstirlitz/luaparse), MIT license — unmodified below this header.
// Why vendored, not a devDependency: the container boot-time npm install is prod-only and prunes dev
// packages from the bind-mounted node_modules on every restart, and lint:lua-syntax is a DEPLOY-TIME
// guard that must run from a bare checkout with no install step. Single file, zero dependencies.
