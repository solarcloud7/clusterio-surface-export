 ...
 * 512 px starmap art — see web/assets/README.md for the regeneration command and the drift hazard.
 *
 * A name that is NOT in here falls back to the spritesheet rather than rendering nothing, so adding
 * a gateway without adding art degrades to "soft but correct" instead of "blank node".
 */
...
export function InstanceNode({ data, isConnectable }: NodeProps) {
	...
	// One-gate nodes drop the circle entirely: the art has its own alpha, so the gateway's silhouette
	// IS the node. A circular mask cropped the pedestal off the bottom of the art and filled the
	// corners with node background, which is the opposite of "the node is the gateway".
	return (
		<div className={
			`surface-export-instance-node${node.online ? "" : " surface-export-instance-node-offline"}`
			...
			{oneGate ? (
				// The gate IS the node: the art fills the whole circle rather than sitting inside it as a
				// thumbnail, so the instance reads as its gateway at a glance. The label moves below the
				// node so nothing is drawn over the art.
				<div
					className={`surface-export-instance-face surface-export-gw-${usageFor(names[0]).outgoing > 0 ? "active" : "idle"}`}
					style={NODE_FACE_ART[names[0]] ? { backgroundImage: `url(${NODE_FACE_ART[names[0]]})` } : undefined}
				...
				>
					...
					{NODE_FACE_ART[names[0]] ? null : <PlanetIcon name={names[0]} size={96} title={names[0]} />}
				</div>
			) : null}
