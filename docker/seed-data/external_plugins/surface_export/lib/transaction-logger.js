...
class TransactionLogger {
    ...
    getTransferSummaries(limit = 50) {
        ...
        for (const [transferId, transfer] of this.plugin.activeTransfers) {
            byId.set(transferId, this.buildTransferSummary(transferId, transfer, this.getLastEventTimestamp(transferId)));
        }
    ...
    async persistTransactionLog(transferId) {
        try {
            ...
            let allLogs = [];
            try {
                const content = await promises_1.default.readFile(this.plugin.transactionLogPath, "utf8");
            ...
            await promises_1.default.writeFile(this.plugin.transactionLogPath, JSON.stringify(allLogs, null, 2), "utf8");
            this.plugin.persistedTransactionLogs = allLogs;
        }
        catch (err) {
            this.plugin.logger.error(`Failed to persist transaction log: ${(0, helpers_1.getErrorMessage)(err)}`);
        }
    }
