...
export class TransactionLogger {
	 ...
	 */
	archiveRecycledTransferId(transferId: string, startedAt: number | null | undefined) {
		const existing = this.plugin.persistedTransactionLogs.find(
			(log: PersistedTransactionLog) => log.transferId === transferId,
		);
		// Same operation re-registering keeps its record (defensive; registration normally
		// happens once).
		if (existing && existing.transferInfo && startedAt
			&& existing.transferInfo.startedAt === startedAt) {
			return;
		}
		if (existing) {
