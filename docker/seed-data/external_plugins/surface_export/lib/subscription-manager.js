...
class SubscriptionManager {
        ...
        this.treeBroadcastLimiter = new lib.RateLimiter({
            ...
            action: () => {
                this.emitTreeUpdate(this.plugin.lastTreeForceName || "player").catch((err) => {
                    this.plugin.logger.error(`Failed to broadcast tree update: ${(0, helpers_1.getErrorMessage)(err)}`);
                });
            },
        });
    }
    ...
    broadcastToSubscribers(filterFn, event) {
        ...
        for (const [link, subscription] of this.plugin.surfaceExportSubscriptions.entries()) {
            ...
            try {
                ...
                this.plugin.logger.warn(`Failed to send event to subscriber, removing stale connection: ${(0, helpers_1.getErrorMessage)(err)}`);
    ...
    emitTransferUpdate(transfer) {
        ...
        const transferSummary = this.plugin.txLogger.buildTransferSummary(transfer.transferId, transfer, this.plugin.txLogger.getLastEventTimestamp(transfer.transferId));
    ...
    emitLogUpdate(transferId, logEvent) {
        ...
        if (activeTransfer) {
            ...
            summary = this.plugin.txLogger.buildDetailedTransferSummary(transferId, activeTransfer, logEvent?.timestampMs || this.plugin.txLogger.getLastEventTimestamp(transferId));
        }
        ...
        const event = new this.messages.SurfaceExportLogUpdateEvent({
            ...
            transferInfo: (transferInfo && typeof transferInfo === "object" ? transferInfo : null),
            summary: (summary && typeof summary === "object" ? summary : null),
        });
        this.broadcastToSubscribers(subscription => (subscription.logs && (!subscription.transferId || subscription.transferId === transferId)), event);
    }
            ...
            for (const transfer of this.plugin.activeTransfers.values()) {
                ...
                try {
                    link.send(new this.messages.SurfaceExportTransferUpdateEvent({
                        revision: this.plugin.transferRevision,
                        generatedAt: Date.now(),
                        transfer: this.plugin.txLogger.buildTransferSummary(transfer.transferId, transfer, this.plugin.txLogger.getLastEventTimestamp(transfer.transferId)),
                    }));
                }
                catch (_err) {
                    this.plugin.surfaceExportSubscriptions.delete(link);
