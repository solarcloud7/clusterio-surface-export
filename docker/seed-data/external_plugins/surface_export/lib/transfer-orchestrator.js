...
function buildImportMetrics(raw) {
    ...
    const result = { total_ticks: Number(input.total_ticks || 0) };
    for (const f of tickFields)
        result[`${f}_ms`] = Math.round(Number(input[`${f}_ticks`] || 0) * helpers_1.TICKS_TO_MS);
    for (const f of countFields)
        result[f] = Number(input[f] || 0);
    return result;
}
function mergeExportMetrics(storedMetrics, runtimeMetrics) {
    const merged = {
        ...(0, helpers_1.normalizeExportMetrics)((storedMetrics || null)),
        ...(0, helpers_1.normalizeExportMetrics)(runtimeMetrics || null),
    };
...
class TransferOrchestrator {
    ...
    async broadcastTransferStatus(transfer, status, color = null) {
        ...
        for (const instanceId of [transfer.sourceInstanceId, transfer.targetInstanceId]) {
            try {
                await this.plugin.controller.sendTo({ instanceId }, msg).catch(() => { });
            }
    ...
    async tryUnlockSource(transferId, transfer) {
        ...
        try {
            const resp = await this.plugin.controller.sendTo({ instanceId: transfer.sourceInstanceId }, new this.messages.UnlockSourcePlatformRequest({
                platformName: transfer.platformName,
            ...
            this.txLogger.logTransactionEvent(transferId, "rollback_failed", `Unlock failed: ${(0, helpers_1.getErrorMessage)(err)}`, { error: (0, helpers_1.getErrorMessage)(err) });
            return (0, helpers_1.getErrorMessage)(err);
        }
    }
    // ── Transfer initiation ─────────────────────────────────────────────
    async transferPlatform(exportId, targetInstanceId, exportMetrics = null, transferStartedAt = null) {
        ...
        this.txLogger.logTransactionEvent(transferId, "transfer_created", `${transfer.platformName}: ${transfer.sourceInstanceName || transfer.sourceInstanceId} → ${transfer.targetInstanceName || targetInstanceId}`, {
            exportMetrics: mergedExportMetrics,
            payloadMetrics,
        });
        ...
        try {
            ...
            const response = await this.plugin.controller.sendTo({ instanceId: targetInstanceId }, new this.messages.ImportPlatformRequest({
                exportId,
            ...
            this.updateTransfer(transfer);
            this.txLogger.logTransactionEvent(transferId, "import_started", `Awaiting validation (timeout: ${VALIDATION_TIMEOUT_MS / 1000}s)`, { transmissionMs });
            ...
            this.logger.error(`Error transferring platform: ${(0, helpers_1.getErrorMessage)(err)}`);
    ...
    async handleImportFailure(transferId, error, transmissionMs) {
        ...
        this.txLogger.logTransactionEvent(transferId, "import_failed", `Import failed: ${error}`, { error, transmissionMs });
        const rollbackError = await this.tryUnlockSource(transferId, transfer);
        if (rollbackError)
            transfer.error = `${transfer.error}; rollback failed: ${rollbackError}`;
        this.updateTransfer(transfer);
    ...
    scheduleValidationTimeout(transferId) {
        ...
        transfer.validationTimeout = setTimeout(async () => {
            ...
            if (!current || current.status !== "awaiting_validation")
                return;
            this.txLogger.logTransactionEvent(transferId, "validation_timeout", "No validation response within 2 minutes", {});
    ...
    async handleTransferValidation(event) {
        ...
        this.txLogger.logTransactionEvent(event.transferId, "validation_received", `Validation: ${event.success ? "SUCCESS" : "FAILED"}`, {
        ...
        try {
            ...
            this.logger.error(`Error handling validation: ${(0, helpers_1.getErrorMessage)(err)}`);
            transfer.status = "error";
            transfer.error = (0, helpers_1.getErrorMessage)(err);
            this.updateTransfer(transfer);
            await this.broadcastTransferStatus(transfer, `Error: ${(0, helpers_1.getErrorMessage)(err)}`, "red");
        }
    }
    async handleValidationSuccess(transferId, transfer) {
        ...
        const deleteResponse = await this.plugin.controller.sendTo({ instanceId: transfer.sourceInstanceId }, new this.messages.DeleteSourcePlatformRequest({
        ...
        if (deleteResponse.success) {
            ...
            this.txLogger.logTransactionEvent(transferId, "transfer_completed", `Completed in ${Math.round(durationMs / 1000)}s`, {
    ...
    async handleValidationFailure(transferId, transfer, validation) {
        ...
        this.txLogger.logTransactionEvent(transferId, "validation_failed", `Validation failed: ${errorMsg}`, { validation });
        ...
        transfer.completedAt = Date.now();
        this.txLogger.logTransactionEvent(transferId, "transfer_failed", `Failed after ${Math.round((transfer.completedAt - transfer.startedAt) / 1000)}s`, {
    ...
    pruneOldTransfers() {
        if (this.plugin.activeTransfers.size <= 100)
            return;
    ...
    async handleStartPlatformTransferRequest(request) {
        ...
        try {
            const t0 = Date.now();
            const exportResponse = await this.plugin.controller.sendTo({ instanceId: sourceInstanceId }, new this.messages.ExportPlatformRequest({
...
//# sourceMappingURL=transfer-orchestrator.js.map