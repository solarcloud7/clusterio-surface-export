...
3. Create external volume (one-time): `docker volume create factorio-client-2117`
4. Start the cluster: `docker compose up -d`

> **Factorio Game Client**: If you need export-data (icon spritesheets), set `FACTORIO_USERNAME` and `FACTORIO_TOKEN` in `.env`. Host-1 downloads the client into the `factorio-client-2117` volume on first startup. This volume persists across `docker compose down -v`.
