...
For local setup, follow the [development guide](setup.md). Use a branch and keep generated artifacts and credentials out of commits.
...
Describe the problem, resulting behavior, verification and remaining limits in the PR. For cargo, identity or recovery changes, use [independent transfer checks](testing.md#example-investigate-cargo-loss): preserve the original failure, compare physical cargo independently, and test ambiguous or repeated replies. A green validator alone does not establish conservation.
