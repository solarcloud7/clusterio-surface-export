...
dx 14) that `pad-transfer-suite` evaluates on a real transfer's destination. The `property` read takes a
dotted path walked by INDEXING only (`temperature`, `burner.remaining_burning_fuel`,
`force.bulk_inserter_capacity_bonus`); `item_count`/`held`/`fluid` cover the rest. Method-shaped reads
(`get_recipe()`, `get_module_inventory()`, circuit sections) keep bespoke meters — extending the walk to
call methods would trade away its injection-unrepresentability.
...
node tools/testkit/cli.mjs probe lab-omnibus-state-v1 'heat-pipe@43,-13:temperature'
...
./tools/rcon.ps1 11 "/test-run <name-filter>"
...
typo'd path THROWS with the engine's own message; an anchor that matches two entities is refused, not
guessed. The check name carries the property path (`heat-pipe.property(temperature)`), so two reads on one
pad are distinguishable in a failure.
