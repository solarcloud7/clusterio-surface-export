...
from script: `tests/instruments/loader-freeze/run-rung.mjs` constructs it on a throwaway clone. The exemplars
this paragraph used to cite do not exist on the live cluster (measured 2026-08-12: no platform named
`lab-omnibus-platform-v1`, zero infinity-containers on the golden `lab-omnibus-state-v1`, whose four native
turbo-loaders have no feed). Operational facts
...
re-enable arms — loaders honor the flag that transport-belts silently drop.] This paragraph used to say
