...
3. Same-logical-segment fidelity for fully compressed belts is not yet guaranteed; BELT-R9 rejects cross-import engine-line identity mapping as the solution.
...
- Exact same-segment placement is an open fidelity gap.
...
- Engine line identity must not be used as a durable source-to-destination restoration key.
...
- Durable documentation explicitly distinguishes global conservation from same-segment fidelity.
