...
### 17. Historical Pre-Activation Fluid Loss (RETIRED 2026-07-21 — superseded by the 2.1 fluid-segment registry)
**RETIRED**: the historical ~15% pre-activation loss and its unisolated "detached ghost buffer" hypothesis are no longer load-bearing. At 2.1.11 the buffer/window duality is gone: `get_fluid_segment_fluid` reads the exact segment total from any member box at any instant, and fluids are restored in the frozen (`disabled_by_script`) world in one `set_fluid_segment_fluid` write per segment before the single exact gate. Frozen restoration is the shipped ordering; there is no measured pre-activation loss to work around. See the fluid section of [docs/factorio-2.0-api-notes.md](factorio-2.0-api-notes.md).
**Key files**: `core/import-completion.lua` (`complete_import_job`), `import_phases/fluid_restoration.lua`, `export_scanners/fluid-registry.lua`
...
**Root Cause**: `EntityHandlers["assembling-machine"]` / `["furnace"]` exported only `inventories`, never their fluidboxes, so chemical-plant reagents and foundry molten metals were dropped while default-handler entities (pipes, tanks, pumps) were fine.
**Status (2026-07-21, 2.1 fluid-segment registry port)**: handlers now emit `data.fluidboxes = InventoryScanner.extract_fluidboxes(entity)` (the registry-capture reader that replaced the dual-mode `extract_fluids`), which **errors loudly when no registry is armed** — a handler that emits `data.fluidboxes` cannot silently degrade to inventory-only capture. The same gap recurred once for the mining-drill handler (acid-fed drill dropped its sulfuric acid), fixed and caught by the `mining-drill-acid-feed` pad audit.
**Key files**: `export_scanners/entity-handlers.lua` (the 7 handler sites that emit `data.fluidboxes`), `export_scanners/fluid-registry.lua`.
**Lesson**: When adding a new entity handler, check whether the type has a fluidbox and emit `fluidboxes`; the fail-loud registry arming makes a forgotten fluid capture a structural error, not silent loss.
...
### 21. Fusion Plasma Handling (RETIRED 2026-07-21 — plasma rides like any fluid)
**RETIRED**: superseded by the 2.1 fluid-segment registry and the owner ruling (2026-07-20/21) that the `engine_owned` classification plays no role. The connection-category exclusion is deleted (`FluidOwnership.is_engine_owned_box`, `collect_engine_owned_segments`, and every `engine_owned` payload/param field are gone); plasma rides transfers like any fluid, and the only lawful fluid subtraction is `write_rejected` (a physical post-write measurement). At 2.1.11 plasma writes stick capacity-clamped (reactor output `set_fluid` 50→10, generator input 25→10); fusion-generator boxes are segmentless. See [docs/factorio-2.0-api-notes.md](factorio-2.0-api-notes.md).

### 22. Activatable Entities Expose No Own Segment ID on 2.0.77 (RETIRED 2026-07-21 — superseded by the 2.1 fluid-segment registry)
**RETIRED**: the per-box nil-segment-id classification and its fusion-reactor refinement were 2.0.77 concerns. At 2.1.11 `entity.fluidbox` is hard-removed and segment getters THROW on segmentless boxes (they no longer return nil), so capture guards every read with `has_fluid_segment(i)` and reads segmentless boxes (fusion-generator boxes, machine buffers) via `get_fluid(i)`. Segment identity is captured once and deduplicated by source segment id; no engine-owned segment set is consulted. See [docs/factorio-2.0-api-notes.md](factorio-2.0-api-notes.md).

### 23. Temperature Merge and Key Boundaries (RETIRED 2026-07-21 — superseded by the 2.1 fluid-segment registry)
**RETIRED**: temperature is now carried per registry segment record (energy-weighted at capture) and validated aggregate-by-name at `epsilon=1e-6`. The 2.0.77 volume-weighted merge measurement and the disproven ">1,000,000°C doubles lose precision" key-boundary story are no longer load-bearing. `loss-analysis.lua reconcile_fluids` still aggregates by name for the UI.
