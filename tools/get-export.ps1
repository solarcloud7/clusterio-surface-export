...
$exportResult = docker exec surface-export-controller npx clusterioctl $ctlConfig --log-level error instance send-rcon $instanceName "/export-platform $PlatformIndex" 2>&1 | Out-String
...
while ($attemptCount -lt $maxAttempts -and -not $exportId) {
    ...
    $listResult = docker exec surface-export-controller npx clusterioctl $ctlConfig --log-level error instance send-rcon $instanceName $listCommand 2>&1 | Out-String
...
$exportDataResult = docker exec surface-export-controller npx clusterioctl $ctlConfig --log-level error instance send-rcon $instanceName $getExportCommand 2>&1 | Out-String
