...
> | Catch swallow | `lint:catch-swallow` | no catch substitutes a default without surfacing the error binding — plugin TS/TSX AND repo-root `tools/`+`tests/` `.mjs` (the runner/testkit/lifecycle-engine dialect); surfacing includes property-writes, outer-variable retry assignment, and `.push` carrying the binding | `// catch:allow` |
