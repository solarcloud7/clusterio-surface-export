...
> `npm run lint` runs nine **correctness** guards, all gated in CI:
...
>
> A tenth guard runs as its own PR-gated CI step (not in `npm run lint`): **commit labels** — `scripts/lint-commit-labels.mjs` fails a PR whose `docs:`-labeled commit touches non-doc paths (commit labels are audit boundaries; a mislabeled rider once evaded two review passes).
>
