...
**Evidence status (corrected 2026-07-06)**: the FIX is **[empirical]** — the pre-activation injection loss was reproducible (~15%), the reorder eliminated it, regression-tested since. The Root-Cause MECHANISM above is **[hypothesis]** — the cited `FluidSystem::*` internals are closed-source and were never inspectable ("expert analysis" was an unverifiable consult, not verification), and the 2026-07 destination-hold CI data contradicts its timing prediction (fluid in a deactivated machine reads as lost BEFORE reactivation, not after the rejoin-merge). Under test in `tests/fluid-lab/`; build on the behavioral rule, not the mechanism. See [factorio-2.0-api-notes.md](docs/factorio-2.0-api-notes.md).
